...
function buildEmail(reqDoc) {
  const brand = reqDoc.organisation || reqDoc.name || "A new brand";
  const subject = `New signup — ${brand} wants to work with 5th Avenue`;
  const ctaUrl = `${APP_URL.replace(/\/$/, "")}/client-requests`;
  ...
  <div style="background:#F5F5F7;padding:32px 16px;font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif">
    <div style="max-width:540px;margin:0 auto;background:#FFFFFF;border-radius:14px;overflow:hidden;border:1px solid #E5E5EA">
      ...
      <div style="padding:28px 32px 8px">
        <p style="margin:0 0 4px;font-size:15px;color:#1D1D1F;font-weight:600">New project brief received</p>
        <p style="margin:0 0 20px;font-size:13.5px;color:#6E6E73;line-height:1.6">
          <strong>${esc(brand)}</strong> just submitted the "Start a project" form on the landing page. Details below —
          set them up as a client whenever you're ready.
        </p>
        ...
        <div style="margin-top:18px;padding:14px 16px;background:#F9F9FB;border-radius:8px;border:1px solid #F0F0F2">
          <div style="font-size:10.5px;font-weight:600;color:#8A8A8E;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:6px">What they want</div>
          <div style="font-size:13px;color:#1D1D1F;line-height:1.6">${esc(reqDoc.goal)}</div>
        </div>` : ""}
      </div>
...
/**
 * Notify the founder of a new client request. Best-effort — resolves to
 * { ok:true } on success, { ok:false, reason } otherwise; never rejects.
 */
export async function sendFounderEmail(reqDoc) {
  ...
  try {
    const { subject, html } = buildEmail(reqDoc);
    ...
    console.log(`[mailer] founder notified of ${reqDoc.id || "signup"} (${data?.id})`);
    return { ok: true };
  } catch (err) {
